...
le.takePicture('long_exposure', 0.5)
...
cam2 = cv2.imread('./data/small_rect_black.jpg') #pic to be warped
...
for i in range(4):
    cv2.circle(dst, (int(corners[i][0]),int(corners[i][1])), 10, (0,0,255), -1)

cv2.imshow('dst',dst)
cv2.imwrite('./data/corners.jpg', dst)
...
'''wrong? if 0 < int(p[0][0]-left) < anamorph.shape[1] and 0 < int(p[1][0]-top) < anamorph.shape[0]:
            anamorph[int(p[1][0]-top)][int(p[0][0]-left)] = cam2[y][x]'''

anamorph = cv2.warpPerspective(cam2, HInv, (width, height))
...
le.takePicture('check_anamorph', 0.5)
