...
pic = cv2.imread('./data/blank.jpg') #pic to be warped
img = cv2.imread('./data/long_exposure.jpg', cv2.CV_LOAD_IMAGE_GRAYSCALE)
...
# src = stored in variable pic; this is the image image we want to see in the camera
# dst = anamorph; pre-warped version of src
'''for y in range(anamorph.shape[0]):
    for x in range(anamorph.shape[1]):
        p = np.array([[x+left],[y+top],[1]]) #([[x],[y],[1]])
        p = np.mat(p) # vertical
        p = H.dot(p)
        p /= p[2]
        
        if 0 < int(p[0][0]) < pic.shape[1] and 0 < int(p[1][0]) < pic.shape[0]:
            anamorph[y][x] = pic[int(p[1][0])][int(p[0][0])]'''
...
for y in range(pic.shape[0]):
    for x in range(pic.shape[1]):
        ...
        if 0 < int(p[0][0]-left) < anamorph.shape[1] and 0 < int(p[1][0]-top) < anamorph.shape[0]:
            anamorph[int(p[1][0]-top)][int(p[0][0]-left)] = pic[y][x]
