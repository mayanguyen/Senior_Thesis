...
chesspoints1, chesspoints2 = cc.chessCorners()
...
screen, corners1 = findCorners(screen)
...
screen, corners2 = findCorners(screen)
...
H1_SC, mask = cv2.findHomography(src_corners, corners1, cv2.RANSAC)
H2_SC, mask = cv2.findHomography(src_corners, corners2, cv2.RANSAC)
#print('H_SC = '+str(H_SC))
...
H1_PC, mask = cv2.findHomography(src_corners, chesspoints1, cv2.RANSAC, 5.0)
H2_PC, mask = cv2.findHomography(src_corners, chesspoints2, cv2.RANSAC, 5.0)
#print('H_PC = '+str(H_PC))

H1Inv_PC = np.linalg.inv(H1_PC)
H2Inv_PC = np.linalg.inv(H2_PC)
