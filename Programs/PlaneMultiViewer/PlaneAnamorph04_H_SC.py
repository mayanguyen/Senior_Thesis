...
# import warp # my implementations of backwarp and forwarp
...
screen = cv2.imread('./data/screen.jpg', cv2.CV_LOAD_IMAGE_GRAYSCALE)
screenColor = cv2.imread('./data/screen.jpg')
...
for i in range(4):
    cv2.circle(screenColor,
        (int(corners[i][0]),int(corners[i][1])), 12, (255,0,0), -1)

cv2.imshow('screen_corners',screenColor)
cv2.imwrite('./data/screen_corners.jpg', screenColor)
...
max_rect = mr.maxRect(screen.shape[1], screen.shape[0], sortCorners(projectionInCamera_corners), image.shape[1], image.shape[0])
chessprojection = cv2.imread('./data/chessProjection.jpg')
for i in range(4):
    cv2.circle(chessprojection,
