...
projectionInCamera_corners1 = cv2.perspectiveTransform(projector_corners, H_PC1)
projectionInCamera_corners2 = cv2.perspectiveTransform(projector_corners, H_PC2)
#print('projection in camera corners = '+str(projectionInCamera_corners))
#for i in range(4):
...
max_rect1 = mr.maxRect(screen.shape[1], screen.shape[0], sortCorners(projectionInCamera_corners1), image.shape[1], image.shape[0])
max_rect2 = mr.maxRect(screen.shape[1], screen.shape[0], sortCorners(projectionInCamera_corners2), image.shape[1], image.shape[0])
'''chessprojection = cv2.imread('./data/chessProjection.jpg')
for i in range(4):
    cv2.circle(chessprojection,
               (int(max_rect[i][0]),int(max_rect[i][1])), 12, (255,0,0), -1)
...
rect_w, rect_h = max_rect1[1][0]-max_rect1[0][0]+1, max_rect1[2][1]-max_rect1[1][1]+1
...
pre_anamorph1 = np.zeros((screen.shape[0], screen.shape[1], 3), np.uint8) # empty black image
pre_anamorph1[max_rect1[0][1]:(max_rect1[0][1]+image_h),
             max_rect1[0][0]:(max_rect1[0][0]+image_w)] = image

cv2.imshow('pre-anamorph1', pre_anamorph1)
cv2.imwrite('./data/test'+str(version)+'/pre_anamorphSingleViewCam1.jpg', pre_anamorph1)

##### Repeat

rect_w, rect_h = max_rect2[1][0]-max_rect2[0][0]+1, max_rect2[2][1]-max_rect2[1][1]+1
image_w, image_h = image.shape[1], image.shape[0]
scale = min(1.*rect_w/image_w, 1.*rect_h/image_h)
...
image = cv2.resize(image, (image_w, image_h))
#pre_anamorph = np.zeros((rect_h+max_rect[0][1],rect_w+max_rect[0][0], 3), np.uint8) # empty black image
pre_anamorph2 = np.zeros((screen.shape[0], screen.shape[1], 3), np.uint8) # empty black image
pre_anamorph2[max_rect2[0][1]:(max_rect2[0][1]+image_h),
             max_rect2[0][0]:(max_rect2[0][0]+image_w)] = image

cv2.imshow('pre-anamorph2', pre_anamorph2)
cv2.imwrite('./data/test'+str(version)+'/pre_anamorphSingleViewCam2.jpg', pre_anamorph2)



########################### Warp (create anamorphic image)
#anamorph = warp.forwarp(image, HInv_PC)
anamorph1 = cv2.warpPerspective(pre_anamorph1, HInv_PC1,
                               (chessboard.shape[1],chessboard.shape[0]))
anamorph2 = cv2.warpPerspective(pre_anamorph2, HInv_PC2,
                               (chessboard.shape[1],chessboard.shape[0]))

cv2.imwrite('./data/test'+str(version)+'/anamorph1.jpg', anamorph1)
cv2.imwrite('./data/test'+str(version)+'/anamorph2.jpg', anamorph2)
cv2.imwrite('./displayFullScreen/data/anamorph1.jpg', anamorph1)
cv2.imwrite('./displayFullScreen/data/anamorph2.jpg', anamorph2)



le.takePicture('check_anamorphSingle1', version, camera_scale)
le.takePicture('check_anamorphSingle2', version, camera_scale)
